...
def get_trend(symbol, interval="15m", limit=20):
    k = get_klines(symbol, interval=interval, limit=limit)
    if not k: return None
    try:
        closes = [float(i["close"]) for i in k]
        if len(closes) < 5: return None
        return "up" if closes[-1] > max(closes[:-1]) else "down" if closes[-1] < min(closes[:-1]) else "sideways"
    except: return None
...
def detect_signal(symbol):
    ...
    if trend15 != trend4h or trend15 is None or trend4h is None: return None
        if last > high * 0.995:
